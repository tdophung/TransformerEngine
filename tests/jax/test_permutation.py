...
def reference_unpermute_with_mask_map(
    ...
    unpermuted_probs = None if permuted_probs is None else jnp.zeros((num_tokens, num_experts), dtype=permuted_probs.dtype)
    ...
    for token_idx in range(num_tokens):
        ...
        for i in range(n_routed):
            ...
            if permuted_probs is not None:
                unpermuted_probs = unpermuted_probs.at[token_idx, expert_idx].set(permuted_probs[src_row])
...
class TestPermutation:
    ...
    def generate_routing_map(
        ...
        if use_fixed_per_token:
            # Old behavior: each token routes to exactly tokens_per_expert experts
            ...
            # PyTorch-style: randomly distribute routing (varying n_routed per token)
    ...
    # Test make_row_id_map
    @pytest.mark.parametrize("num_tokens,num_experts,tokens_per_expert", [
        (32, 8, 2),
        (64, 16, 3),
        (128, 8, 1),
    ])
    ...
    def test_make_row_id_map(self, num_tokens, num_experts, tokens_per_expert, use_fixed_per_token):
        ...
        # print("\n" + "="*100)
        # print(f"TEST: make_row_id_map (num_tokens={num_tokens}, num_experts={num_experts}, tokens_per_expert={tokens_per_expert})")
        # print("="*100)
        
        # print("\n📊 ROUTING MAP (rows=tokens, cols=experts):")
        # print("-"*100)
        # print(routing_map)
        
        # print("\n📋 FULL ARRAYS:")
        # print("-"*100)
        # print("\n🔴 ACTUAL (JAX/Triton implementation) - AFTER PASS 3:")
        # print(test_row_id_map)
        # print("\n🔴 ACTUAL - Columns breakdown:")
        # print(f"  Sorted dest rows [0:{num_experts}]:")
        # print(test_row_id_map[:, :num_experts])
        # print(f"  Expert indices [{num_experts}:{2*num_experts}]:")
        # print(test_row_id_map[:, num_experts:2*num_experts])
        # print(f"  n_routed (last column):")
        # print(test_row_id_map[:, -1])
        
        # print("\n🟢 EXPECTED (Reference implementation):")
        # print(ref_row_id_map)
        # print("\n🟢 EXPECTED - Columns breakdown:")
        # print(f"  Sorted dest rows [0:{num_experts}]:")
        # print(ref_row_id_map[:, :num_experts])
        # print(f"  Expert indices [{num_experts}:{2*num_experts}]:")
        # print(ref_row_id_map[:, num_experts:2*num_experts])
        # print(f"  n_routed (last column):")
        # print(ref_row_id_map[:, -1])
        
        # print("\n🔍 DIFFERENCE (Actual - Expected):")
        # diff = test_row_id_map - ref_row_id_map
        # print(diff)

        # mismatch_count = jnp.sum(diff != 0)
        # total_elements = test_row_id_map.size
        # print(f"\n📊 STATISTICS:")
        # print(f"  Total elements: {total_elements}")
        # print(f"  Mismatched elements: {mismatch_count} ({100*mismatch_count/total_elements:.1f}%)")
        # print(f"  Max absolute difference: {jnp.max(jnp.abs(diff))}")
        
        # print("\n" + "="*100 + "\n")
        
        # Compare results - only compare valid positions (first n_routed in each section)
        # Invalid positions may contain garbage (PyTorch) or -1 (JAX reference), but they're never accessed
        for token_idx in range(num_tokens):
        ...
        # # Optional: Also do a full comparison if both use -1 for invalid positions
        # # This will help catch uninitialized memory issues
        # if jnp.all((test_row_id_map == -1) | (test_row_id_map >= 0)):
        #     print("🔬 Both use -1 for invalid positions, doing full comparison...")
        #     assert_allclose(test_row_id_map, ref_row_id_map, rtol=0, atol=0)
    
    # Test permute_with_mask_map
    @pytest.mark.parametrize("num_tokens,num_experts,hidden_size,tokens_per_expert", [
        (32, 8, 256, 2),
        (64, 16, 512, 3),
        # Add smaller test cases for easier debugging
        (16, 4, 64, 2),  # Small case for debugging
        (8, 2, 32, 1),   # Minimal case
    ])
    @pytest.mark.parametrize("dtype", [jnp.float32, jnp.bfloat16])
    @pytest.mark.parametrize("with_probs", [True, False])
    def test_permute_with_mask_map(self, num_tokens, num_experts, hidden_size, tokens_per_expert, dtype, with_probs):
        ...
        inp = jax.random.uniform(inp_key, (num_tokens, hidden_size), dtype=dtype, minval=-1.0, maxval=1.0)
        
        if with_probs:
            probs = jax.random.uniform(prob_key, (num_tokens, num_experts), dtype=dtype, minval=0.0, maxval=1.0)
        else:
            probs = None
        ...
        # Debug output for bfloat16 failures
        if dtype == jnp.bfloat16 and with_probs:
            print(f"\n{'='*100}")
            print(f"DEBUG: test_permute_with_mask_map (dtype=bfloat16, with_probs=True)")
            print(f"  num_tokens={num_tokens}, num_experts={num_experts}, hidden_size={hidden_size}")
            print(f"  num_out_tokens={num_out_tokens}, tokens_per_expert={tokens_per_expert}")
            print(f"{'='*100}")
            
            # Check output differences (convert to float32 for printing)
            output_diff = jnp.abs(test_output.astype(jnp.float32) - ref_output.astype(jnp.float32))
            print(f"\n📊 OUTPUT DIFFERENCES:")
            print(f"  Max diff: {float(jnp.max(output_diff)):.6f}")
            print(f"  Mean diff: {float(jnp.mean(output_diff)):.6f}")
            print(f"  Median diff: {float(jnp.median(output_diff)):.6f}")
            print(f"  Num elements with diff > 0.1: {int(jnp.sum(output_diff > 0.1))}")
            print(f"  Num elements with diff > 0.5: {int(jnp.sum(output_diff > 0.5))}")
            print(f"  Num elements with diff > 0.9: {int(jnp.sum(output_diff > 0.9))}")
            
            # Find worst mismatches
            flat_diff = output_diff.flatten()
            worst_indices = jnp.argsort(flat_diff)[-10:]  # Top 10 worst
            print(f"\n🔍 WORST MISMATCHES (flattened indices):")
            for i, idx in enumerate(worst_indices):
                row = int(idx // hidden_size)
                col = int(idx % hidden_size)
                actual_val = float(test_output.flatten()[idx])
                expected_val = float(ref_output.flatten()[idx])
                diff_val = float(flat_diff[idx])
                print(f"  [{i}] position=({row},{col}), actual={actual_val:.4f}, "
                      f"expected={expected_val:.4f}, diff={diff_val:.4f}")
            
            # Check probs differences if present
            if test_probs is not None and ref_probs is not None:
                prob_diff = jnp.abs(test_probs.astype(jnp.float32) - ref_probs.astype(jnp.float32))
                print(f"\n📊 PROBS DIFFERENCES:")
                print(f"  Max diff: {float(jnp.max(prob_diff)):.6f}")
                print(f"  Mean diff: {float(jnp.mean(prob_diff)):.6f}")
                print(f"  Num elements with diff > 0.1: {int(jnp.sum(prob_diff > 0.1))}")
            
            # Check input data quality
            print(f"\n📊 INPUT DATA STATS:")
            print(f"  inp range: [{float(jnp.min(inp)):.4f}, {float(jnp.max(inp)):.4f}]")
            print(f"  inp has NaN: {bool(jnp.any(jnp.isnan(inp)))}")
            if probs is not None:
                print(f"  probs range: [{float(jnp.min(probs)):.4f}, {float(jnp.max(probs)):.4f}]")
            print(f"\n{'='*100}\n")
    ...
    def test_unpermute_with_mask_map(
        ...
        with_merging_probs, with_permuted_probs
    ):
        ...
        inp = jax.random.uniform(inp_key, (num_out_tokens, hidden_size), dtype=dtype, minval=-1.0, maxval=1.0)
        
        if with_merging_probs:
            merging_probs = jax.random.uniform(merge_key, (num_tokens, num_experts), dtype=dtype, minval=0.0, maxval=1.0)
        ...
        if with_permuted_probs:
            permuted_probs = jax.random.uniform(prob_key, (num_out_tokens,), dtype=dtype, minval=0.0, maxval=1.0)
        else:
            permuted_probs = None
    ...
    def test_permute_unpermute_roundtrip(self, num_tokens, num_experts, hidden_size, tokens_per_expert, dtype):
        ...
        merging_probs = routing_map.astype(dtype) / jnp.maximum(jnp.sum(routing_map, axis=1, keepdims=True), 1.0)
    ...
    # Test make_chunk_sort_map
    @pytest.mark.parametrize("num_splits,total_tokens", [
        (4, 128),
        (8, 256),
        (16, 512),
    ])
    def test_make_chunk_sort_map(self, num_splits, total_tokens):
        ...
        # Reference implementation
        ref_map = reference_make_chunk_sort_map(split_sizes, sorted_indices, total_tokens, num_splits)
        
        # Compare results
        assert_allclose(test_map, ref_map, rtol=0, atol=0)
    
    # Test sort_chunks_by_map
    @pytest.mark.parametrize("num_splits,total_tokens,hidden_size", [
        (4, 128, 256),
        (8, 256, 512),
    ])
    @pytest.mark.parametrize("dtype", [jnp.float32, jnp.bfloat16])
    ...
    @pytest.mark.parametrize("with_probs", [True, False])
    def test_sort_chunks_by_map(self, num_splits, total_tokens, hidden_size, dtype, is_forward, with_probs):
        ...
        # Generate input data
        key, inp_key, prob_key = jax.random.split(key, 3)
        inp = jax.random.uniform(inp_key, (total_tokens, hidden_size), dtype=dtype, minval=-1.0, maxval=1.0)
        
        if with_probs:
            probs = jax.random.uniform(prob_key, (total_tokens,), dtype=dtype, minval=0.0, maxval=1.0)
        else:
            probs = None
        
        # Debug prints
        # print("\n" + "="*100)
        # print(f"TEST: sort_chunks_by_map (num_splits={num_splits}, total_tokens={total_tokens}, hidden_size={hidden_size})")
        # print(f"      dtype={dtype}, is_forward={is_forward}, with_probs={with_probs}")
        # print("="*100)
        ...
        # print(f"row_id_map (first 10): {row_id_map[:10]}")
        # print(f"inp.shape: {inp.shape}, inp.dtype: {inp.dtype}")
        # print(f"inp[0, :5]: {inp[0, :5]}")  # First 5 elements of first row
        # if probs is not None:
        #     print(f"probs.shape: {probs.shape}, probs.dtype: {probs.dtype}")
        #     print(f"probs[:5]: {probs[:5]}")
        ...
        # print("\n📋 OUTPUT COMPARISON:")
        # print("-"*100)
        # print(f"\n🔴 ACTUAL (Triton implementation):")
        # print(f"test_output.shape: {test_output.shape}, test_output.dtype: {test_output.dtype}")
        # print(f"test_output[0, :5]: {test_output[0, :5]}")  # First 5 elements of first row
        # print(f"test_output has NaN: {jnp.any(jnp.isnan(test_output))}")
        # print(f"test_output has Inf: {jnp.any(jnp.isinf(test_output))}")
        # if test_probs is not None:
        #     print(f"test_probs[:5]: {test_probs[:5]}")
        
        # print(f"\n🟢 EXPECTED (Reference implementation):")
        # print(f"ref_output.shape: {ref_output.shape}, ref_output.dtype: {ref_output.dtype}")
        # print(f"ref_output[0, :5]: {ref_output[0, :5]}")  # First 5 elements of first row
        # if ref_probs is not None:
        #     print(f"ref_probs[:5]: {ref_probs[:5]}")
        
        # print("\n🔍 DIFFERENCE (Actual - Expected):")
        # if not jnp.any(jnp.isnan(test_output)):
        #     diff = test_output - ref_output
        #     print(f"Max absolute difference: {jnp.max(jnp.abs(diff))}")
        #     print(f"Mean absolute difference: {jnp.mean(jnp.abs(diff))}")
        # else:
        #     print("Cannot compute difference - test_output contains NaN values")
