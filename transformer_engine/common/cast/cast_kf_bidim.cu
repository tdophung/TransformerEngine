...
constexpr uint32_t kMinExponentField = 0x0400u;
// Encoding of 2^135; subtracting the rounded exponent field yields the
// reciprocal scale 2^(135 - e) directly.
constexpr uint32_t kReciprocalBias = 0x8300u;
...
__device__ __forceinline__ uint32_t mx_scale_reciprocal(uint32_t amax_bits) {
  ...
  const uint32_t rounded_exponent =
      max((amax_bits + kMantissaRoundUp) & kBf16ExponentMask, kMinExponentField);
  return kReciprocalBias - rounded_exponent;
}
...
__device__ __forceinline__ uint32_t mx_scale_reciprocal_x2(uint32_t amax_pair) {
  ...
  constexpr uint32_t kMinExponentPair = 0x04000400u;
  constexpr uint32_t kReciprocalBiasPair = 0x83008300u;
 ...
 */
template <int32_t COLS_PER_LANE, int32_t K_COMPILE_TIME, int32_t MIN_BLOCKS_PER_SM>
...
#pragma unroll
    ...
    const uint32_t reciprocal =
        mx_scale_reciprocal(fold_pair_magnitude(reinterpret_cast<const uint32_t &>(amax)));
...
#pragma unroll
    ...
    scale_and_convert(tile[i], scales, out_words);
...
#pragma unroll
    ...
    if (__builtin_expect(fold_pair_magnitude(amax_pair) < kBf16InfBits, 1)) {
      reciprocal_pair = mx_scale_reciprocal_x2(amax_pair);
    } else {
      // At least one of the two columns is Inf or NaN; take them separately.
      const uint32_t lo = mx_scale_reciprocal(amax_pair & kBf16MagnitudeMask);
      const uint32_t hi = mx_scale_reciprocal((amax_pair >> 16) & kBf16MagnitudeMask);
...
#pragma unroll
  for (int32_t i = 0; i < kRowsPerWarp; ++i) {
    uint32_t out_words[kOutWordsPerLane];
    scale_and_convert(tile[i], colwise_scales, out_words);
 ...
 */
template <int32_t COLS_PER_LANE, int32_t MIN_BLOCKS_PER_SM>
void launch_tiled(const void *input, void *output_rowwise, void *scales_rowwise,
  ...
  cluster_attr.val.clusterDim.x = cluster_width;
...
#define NVTE_LAUNCH_BIDIM(K_CONST)                                                            \
  do {                                                                                        \
    auto kernel = quantize_bidim_kernel<COLS_PER_LANE, K_CONST, MIN_BLOCKS_PER_SM>;           \
    if (cluster_width > 1) {                                                                  \
...
void launch_mxfp8_kf_bidim(const void *input, void *output_rowwise, void *scales_rowwise,
  ...
  if (cols % kWideColsPerTile == 0) {
    ...
    if (wide_ctas < static_cast<int64_t>(kNarrowFromWaves) * wide_resident) {
      launch_tiled<kWideColsPerLane, kWideMinBlocksPerSm>(
  ...
  launch_tiled<kNarrowColsPerLane, kNarrowMinBlocksPerSm>(
      input, output_rowwise, scales_rowwise, output_colwise, scales_colwise, rows, cols,
