...
def pack_swiglu_pair(gate: jax.Array, up: jax.Array) -> jax.Array:
    """Interleave 32-column gate/up blocks as required by the kernels."""
...
_DSWIGLU_OUTPUT_ROLES = ("d", "d_col", "sfd_row", "sfd_col", "dprob")


def _dswiglu_output_avals(a_aval, b_aval, c_aval, output_dtype):
    if a_aval.ndim != 3 or a_aval.shape[-1] != 1:
        raise ValueError(f"Expected A[M,K,1], got {a_aval.shape}")
    if b_aval.ndim != 3:
        raise ValueError(f"Expected physical B[E,N,K], got {b_aval.shape}")
    ...
    if k_a != k_b:
        raise ValueError(f"A K={k_a} does not match B K={k_b}")
    if c_aval.shape != (m, 2 * n, 1):
        raise ValueError(f"Expected C shape {(m, 2 * n, 1)}, got {c_aval.shape}")
    if m % 256:
        raise ValueError(f"Padded activation rows M={m} must be divisible by 256")
    return (
        jax.core.ShapedArray((m, 2 * n, 1), output_dtype),
        jax.core.ShapedArray((m, 2 * n, 1), output_dtype),
        jax.core.ShapedArray((_scale_size(m, 2 * n, colwise=False),), jnp.float8_e8m0fnu),
        jax.core.ShapedArray((_scale_size(m, 2 * n, colwise=True),), jnp.float8_e8m0fnu),
        jax.core.ShapedArray((m, 1, 1), jnp.float32),
    )


def _compile_and_register_dswiglu(avals_in, avals_out):
    try:
        import jax_tvm_ffi
        import tvm_ffi
        from cudnn import (
        ...
        raise RuntimeError(
            "TVM-FFI grouped dSwiGLU requires the patched cuDNN-FE compiler package, "
            "apache-tvm-ffi, and jax-tvm-ffi"
        ) from exc
    ...
    operands = {}
    for role, aval in zip(_DSWIGLU_INPUT_ROLES, avals_in):
        shape = aval.shape
        if role == "b":
            # Present the compact [E,N,K] XLA buffer to the compiler as the
            # kernel's logical [N,K,E] tensor without a runtime transpose.
            shape = (aval.shape[1], aval.shape[2], aval.shape[0])
        operands[role] = GroupedGemmOperandDesc(
            role=role,
            dtype=_dtype_name(aval.dtype),
    ...
    for role, aval in zip(_DSWIGLU_OUTPUT_ROLES, avals_out):
        operands[role] = GroupedGemmOperandDesc(
            role=role,
            dtype=_dtype_name(aval.dtype),
            shape=aval.shape,
    ...
    function, abi = compile_grouped_gemm_dswiglu(
        operands=operands,
        config=GroupedGemmDswigluConfig(
            sf_vec_size=32,
            mma_tiler_mn=(256, 256),
            cluster_shape_mn=(2, 1),
            discrete_col_sfd=True,
    ...
    if not isinstance(function, tvm_ffi.Function):
        raise TypeError(
            "cuDNN-FE compile_grouped_gemm_dswiglu returned a Python wrapper; "
            "the JAX execution target must be a native tvm_ffi.Function"
        )
    expected = [(role, "arg") for role in _DSWIGLU_INPUT_ROLES] + [
        (role, "ret") for role in _DSWIGLU_OUTPUT_ROLES
    ] + [("stream", "stream")]
    actual = [(entry.role, entry.kind) for entry in abi.entries]
    if actual != expected:
        raise ValueError(f"Unsupported grouped dSwiGLU ABI: expected {expected}, got {actual}")

    target_name = f"te_grouped_gemm_dswiglu.{abi.key.rsplit('.', maxsplit=1)[-1]}"
    with _REGISTRATION_LOCK:
        if target_name not in _REGISTERED_TARGETS:
            jax_tvm_ffi.register_ffi_target(
                target_name,
...
class GroupedGemmDswigluPrimitive(BasePrimitive):
    """Fused grouped GEMM, dSwiGLU, and dual MXFP8 quantization."""

    name = "te_grouped_gemm_dswiglu_tvm_ffi"
    multiple_results = True
    impl_static_args = (10,)
    inner_primitive = None
    outer_primitive = None
    ...
    def abstract(
        ...
        *,
        ...
        del sfa_aval, sfb_aval, alpha_aval, beta_aval, norm_const_aval
        m = a_aval.shape[0]
        experts = b_aval.shape[0]
        if padded_offsets_aval.shape != (experts,):
            raise ValueError(
        ...
        if prob_aval.shape != (m, 1, 1):
            raise ValueError(f"Expected prob shape {(m, 1, 1)}, got {prob_aval.shape}")
        return _dswiglu_output_avals(a_aval, b_aval, c_aval, output_dtype)

    @staticmethod
    def lowering(ctx, *args, output_dtype):
        del output_dtype
        target_name, abi = _compile_and_register_dswiglu(ctx.avals_in, ctx.avals_out)
        operand_layouts = [entry.stride_order for entry in abi.entries if entry.kind == "arg"]
        result_layouts = [entry.stride_order for entry in abi.entries if entry.kind == "ret"]
        return jax.ffi.ffi_lowering(
            target_name,
    ...
    def impl(
        ...
        if GroupedGemmDswigluPrimitive.inner_primitive is None:
            raise RuntimeError("GroupedGemmDswigluPrimitive has not been registered")
        return GroupedGemmDswigluPrimitive.inner_primitive.bind(
            a,
            b,
            c,
            sfa,
    ...
    @staticmethod
    def batcher(batched_args, batch_dims, *, output_dtype):
        del batched_args, output_dtype
        raise NotImplementedError(
            f"GroupedGemmDswigluPrimitive does not support vmap batch dimensions {batch_dims}"
        )

    @staticmethod
    def partition(output_dtype, mesh, arg_infos, result_infos):
        del result_infos
        a_spec = get_padded_spec(arg_infos[0])
        m_axis = a_spec[0]
        experts = arg_infos[1].shape[0]
        ...
        def sharded_impl(
            ...
            if experts == 1:
                padded_offsets = jnp.asarray([a.shape[0]], dtype=jnp.int32)
            return GroupedGemmDswigluPrimitive.impl(
                a,
                b,
                c,
                sfa,
    ...
    def shardy_sharding_rule(*args, **kwargs):
        del args, kwargs
        return (
            "m k one, experts n k, m two_n one, sfa, sfb, experts, experts, experts, "
            "m prob_one_a prob_one_b, norm -> "
            "m two_n one, m two_n one, sfd_row, sfd_col, m prob_one_a prob_one_b"
        )


register_primitive(GroupedGemmDswigluPrimitive)
...
def grouped_gemm_dswiglu(
    ...
    *,
    ...
    """Run the TVM-FFI fused grouped dSwiGLU primitive.

    ``b`` uses TE's compact physical ``[E,N,K]`` representation. The returned
    dprob buffer is not consumed because the kernel accumulates into it
    atomically; the MoE VJP computes the routing cotangent explicitly.
    """
    if b.ndim != 3:
        raise ValueError(f"Expected physical B[E,N,K], got {b.shape}")
    experts = b.shape[0]
    ...
    norm_const = jnp.ones((1,), dtype=jnp.float32)
    return GroupedGemmDswigluPrimitive.outer_primitive.bind(
        a,
        b,
        c,
        sfa.reshape(-1),
